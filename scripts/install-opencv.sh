...
# |----------------|--------------|------|-------------|
# | Ubuntu 16.04.2 | OpenCV 3.3.1 | OK   | 01 Nov 2017 |
...
mkdir build
cd build
cmake -DWITH_QT=ON -DWITH_OPENGL=ON -DFORCE_VTK=ON -DWITH_TBB=ON -DWITH_GDAL=ON -DWITH_XINE=ON -DBUILD_EXAMPLES=ON -DENABLE_PRECOMPILED_HEADERS=OFF -DBUILD_JASPER=ON ..
make -j4
sudo make install
sudo ldconfig
